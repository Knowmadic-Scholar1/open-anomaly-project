...
test('the voice TOOL SCHEMA is byte-identical to main — the mission mapping is instructions only', () => {
  ...
  // Re-pinned 2026-09-10: Open Anomaly Project adds `oap-anomalies` to
  // set_layer_visibility / show_data_layers_menu enums (deliberate schema
  // extension). First-run missions still ride existing tools only.
  assert.equal(block.length, 31278, 'tool schema byte length drifted from the pinned release schema');
  assert.equal(
    crypto.createHash('sha256').update(block).digest('hex'),
    'fae26ea8c6431ea9587c1be72c5a0632c031e4ecf4f2dabf71b4a7cd37d5ffc8',
